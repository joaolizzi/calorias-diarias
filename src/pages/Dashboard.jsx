...
import { getProfile, getWaterForDay, getFoodForDay } from '../lib/supabase.js';
...
export default function Dashboard({ theme, accent, setTheme, setAccent }) {
  ...
  const reloadProfile = useCallback(async () => {
    try { setProfile(await getProfile(user.id)); } catch (e) { console.warn('getProfile falhou:', e?.message); }
  }, [user.id]);

  const reloadToday = useCallback(async () => {
    try {
      const [w, f] = await Promise.all([getWaterForDay(user.id, day), getFoodForDay(user.id, day)]);
      setWaterEntries(w); setFoodEntries(f);
  ...
  return <div className="app">
    <ProfessionalHeader subtitle={`${fmtDateLabel(day)} · hoje`} theme={theme} accent={accent} setTheme={setTheme} setAccent={setAccent} />
    <Toast />
