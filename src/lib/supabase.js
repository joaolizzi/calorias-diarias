...
export const ensureProfile = async (userId) => {
  ...
  if (!existing) {
    const { error } = await supabase.from('profiles').insert({ id: userId, daily_kcal_goal: 2000, daily_water_goal_ml: 2000, onboarding_complete: false });
    if (error && error.code !== '23505') throw error;
  }
};
export const getProfile = async (userId) => { const { data, error } = await supabase.from('profiles').select('*').eq('id', userId).maybeSingle(); if (error) throw error; return data; };
export const updateGoals = async (userId, { kcalGoal, waterGoal }) => { const updates = {}; if (kcalGoal != null) updates.daily_kcal_goal = Math.round(Number(kcalGoal)); if (waterGoal != null) updates.daily_water_goal_ml = Math.round(Number(waterGoal)); const { error } = await supabase.from('profiles').update(updates).eq('id', userId); if (error) throw error; };
export const saveGoalProfile = async (userId, payload) => { const updates = { display_name: payload.displayName || null, weight_kg: Number(payload.weightKg), height_cm: Number(payload.heightCm), age: Number(payload.age), sex: payload.sex, activity_level: payload.activity, goal: payload.goal, daily_kcal_goal: Math.round(Number(payload.kcalGoal)), daily_water_goal_ml: Math.round(Number(payload.waterGoal)), onboarding_complete: true }; const { error } = await supabase.from('profiles').update(updates).eq('id', userId); if (error) throw error; };
...
// ---------- comida ----------
export const getFoodForDay = async (userId, day) => { const { data, error } = await supabase.from('food_entries').select('*').eq('user_id', userId).eq('day', day).order('consumed_at', { ascending: true }); if (error) throw error; return data || []; };
export const addFood = async (userId, day, { meal, name, kcal, grams }) => { const { error } = await supabase.from('food_entries').insert({ user_id: userId, day, meal, name, kcal, grams }); if (error) throw error; };
...
export const getFoodRange = async (userId, fromDay, toDay) => { const { data, error } = await supabase.from('food_entries').select('kcal, day').eq('user_id', userId).gte('day', fromDay).lte('day', toDay); if (error) throw error; return data || []; };
